...
class Evaluator(object):
        ...
        if new_cols <= crop_size[1] or new_rows <= crop_size[0]:
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    if temp_score.dim() == 4 and temp_score.shape[0] == 1:  # 自己添加
                        temp_score = temp_score.squeeze(0)  # 从 [1, 2, H, W] -> [2, H, W]
    ...
    def val_func_process_rgbX(self, input_data, input_modal_x, device=None):
        ...
        with torch.cuda.device(input_data.get_device()):
            ...
            with torch.no_grad():  # 自己修改
                score = self.val_func(input_data, input_modal_x)
                # 如果 val_func 返回的是一个 tuple，比如 (x_last, contrastive_loss)，取第一个
                if isinstance(score, (tuple, list)):
                    score = score[0]
                # 保证 score 最终是 [C, H, W]
                if score.dim() == 4 and score.shape[0] == 1:
                    score = score.squeeze(0)
                if self.is_flip:
                    input_data = input_data.flip(-1)
                    input_modal_x = input_modal_x.flip(-1)
                    score_flip = self.val_func(input_data, input_modal_x)
                    if isinstance(score_flip, (tuple, list)):
                        score_flip = score_flip[0]
                    if score_flip.dim() == 4 and score_flip.shape[0] == 1:
                        score_flip = score_flip.squeeze(0)
                    score += score_flip.flip(-1)
                score = torch.exp(score)
