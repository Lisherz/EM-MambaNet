...
if True:
    ...
    def selective_scan_1d(
        ...
        if softmax_version:
            # y = y.softmax(y, dim=-1).to(x.dtype)
            y = ys.softmax(ys, dim=-1).to(x.dtype)
...
class ImprovedCrossMambaFusionBlock(nn.Module):  #CFB
    def __init__(self, hidden_dim, drop_path=0.0, mlp_ratio=0.0, **kwargs):
        super().__init__()
        self.use_checkpoint = kwargs.get('use_checkpoint', False)
        self.cross_attn = CrossMambaFusion_SS2D_SSM(hidden_dim, **kwargs)  # 原始交叉融合
        self.modal_weight_rgb = nn.Parameter(torch.tensor(0.5))  # SAR权重
        self.modal_weight_e = nn.Parameter(torch.tensor(0.5))  # 光学权重
        # SAR权重和光学权重  cal,gl数据集为 0.7 0.3
        self.drop_path = DropPath(drop_path)
        if mlp_ratio > 0:
            self.mlp = Mlp(hidden_dim, int(hidden_dim * mlp_ratio))

    def _forward(self, x_rgb, x_e):
        # Cross Attention
        x_rgb_cross, x_e_cross = self.cross_attn(x_rgb, x_e)

        # Modal Weight Fusion
        x_rgb = self.modal_weight_rgb * x_rgb_cross + (1 - self.modal_weight_rgb) * x_rgb
        x_e = self.modal_weight_e * x_e_cross + (1 - self.modal_weight_e) * x_e

        # DropPath Regularization
        x_rgb = x_rgb + self.drop_path(x_rgb_cross)
        x_e = x_e + self.drop_path(x_e_cross)
...
class Backbone_VSSM(VSSM):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            # layer_forward 是一个嵌套函数，用于执行层的 blocks 和 downsample 操作。 输出 o 和下一阶段的输入 x。
            o, x = layer_forward(layer, x) # (B, H, W, C)
            # 如果当前层在 out_indices 中，提取并归一化输出
