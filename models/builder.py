...
logger = get_logger()
# 整个网络模型的运行流程从编码器开始，输入图像（可能是RGB图像和其他模态图像）首先通过骨干网络提取特征。
# 提取的特征图被传递给解码器，并生成语义分割图。若启用了辅助头（如aux_head），则会生成额外的中间预测图。
# 在训练时，网络会根据标签计算损失，并根据配置的条件选择是否启用对比损失。最终，模型的输出包括语义分割图和或辅助分支输出。
# 如果启用了深度监督，模型会返回多个阶段的输出并计算每个输出的损失。
...
class EncoderDecoder(nn.Module):
    ...
    #     outs_fused, contrastive_loss = self.backbone(rgb, modal_x, label)
    #     out = self.decode_head(outs_fused)
    #     return out, contrastive_loss

    def forward(self, rgb, modal_x, label=None):
        outs_fused, edge_loss, contrastive_loss = self.backbone(rgb, modal_x, label)
        # outs_fused, edge_loss  = self.backbone(rgb, modal_x, label)
        if not self.deep_supervision:
            out = self.decode_head(outs_fused)
            if label is not None:
                seg_loss = self.criterion(out, label.long())
                print(seg_loss)
                print(edge_loss)
                print(contrastive_loss)
                total_loss = 1.0 * seg_loss + 0.3 * edge_loss + 0.2 * contrastive_loss  # 加权组合损失
                # total_loss = 1.0 * seg_loss + 0.5 * edge_loss  # 加权组合损失
                return total_loss     # 返回一个标量 loss
