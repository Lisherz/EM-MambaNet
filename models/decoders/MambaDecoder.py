...
class MambaDecoder(nn.Module):
    def __init__(self, 
        ...
        for i_layer in range(self.num_layers):
            if i_layer == 0:
                ...
                # PatchExpand 用于最初的上采样层，将低分辨率特征图扩展到较高分辨率。
                ...
                # Mamba_up负责每一层的特征扩展和融合，并使用了LayerNorm和DropPath等技术以稳定训练过程。
        ...
        # FinalUpsample_X4用于最终的上采样操作，将特征图上采样到目标分辨率。最后的输出层通过nn.Conv2d转换为预测的类别图。
    ...
    # 该函数是解码器的核心部分，负责将通过backbone（如视觉变换器）提取的特征图进行上采样并逐层融合。
    def forward_up_features(self, inputs):  # B, C, H, W    # print(len(inputs))  inputs:2  没有引入对比学习时为4
    ...
    def forward(self, inputs, contrastive_loss=None):
        if not self.deep_supervision:
            x = self.forward_up_features(inputs)
            # x_last = self.up(x)
            x_last = self.up_x4(x, self.patch_size)
            if contrastive_loss is not None:
                return x_last, contrastive_loss
            return x_last
        else:
            x, x_upsample = self.forward_up_features(inputs)
            x_last = self.up(x)
            if contrastive_loss is not None:
                return x_last, x_upsample, contrastive_loss
            return x_last, x_upsample
