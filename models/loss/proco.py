...
# LogRatioC是自定义的PyTorch反向传播函数，继承自torch.autograd.Function。forward和backward方法实现了前向和反向传播。
...
# EstimatorCV类用于估计类别的协方差和均值。它用于估计对比学习中的类别信息。
class EstimatorCV():
...
# ProCoLoss类是自定义的对比损失函数，继承自nn.Module。
class ProCoLoss(nn.Module):
    ...
    def forward(self, features, labels=None, sup_logits=None, args=None):
        ...
        Ave = self.estimator_old.Ave.detach().to(device)
        Ave_norm = F.normalize(Ave, dim=1)
        logc = self.estimator_old.logc.detach().to(device)
        kappa = self.estimator_old.kappa.detach().to(device)

        tem = kappa.reshape(-1, 1) * Ave_norm
        tem = tem.unsqueeze(0) + features[:N].unsqueeze(1) / self.temperature   # 报错了!
