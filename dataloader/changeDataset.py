...
class ChangeDataset(data.Dataset):
    def __init__(self, setting, split_name, preprocess=None):
        ...
        self._file_names = self._get_file_names(split_name)   # 数据集划分的名称，如 train、val 或 test
    ...
    def __getitem__(self, index):
        ...
        # 将图像A、图像B、标签图像、文件名和数据集长度打包成字典返回
    ...
    # get_class_colors：类方法，生成颜色映射表，用于可视化
    @classmethod
...
if __name__=='__main__':
    data_setting = {'root': '/mnt/newdisk/zyy/data/multi/ShuGuang',
                    'A_format': '.png',
                    'B_format': '.png',
                    'gt_format': '.png',
                    'class_names': ['Background', 'Change']}
    
    dataset = ChangeDataset(data_setting, 'train')
    item = dataset[0]
